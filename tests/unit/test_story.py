...
@pytest.mark.usefixtures("mock_db_session", "test_data")
class TestStoryDataStore:
    def test_get_story(self, mock_db_session, test_data):
    ...
    def test_get_not_exist_story(self, mock_db_session):
        story = mock_db_session.get_story("not_exist_id")
        assert story is None

    def test_get_story_with_response_error(self, mock_db_session):
    ...
    def test_get_node(self, mock_db_session, test_data):
        node = mock_db_session.get_node("test_id", "Root")
        assert Node(**test_data["nodes"]["Root"]) == node

    def test_get_not_exist_node(self, mock_db_session):
